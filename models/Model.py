...
from .SimplifiedGNN import SimplifiedGNN  # 引入 Simplified GNN 模型
...
class CIKGRec(torch.nn.Module):
    def __init__(self, config, edge_index):
        ...
        # Initialize the GNN Encoder based on config
        if config['gnn_type'] == 'simplified':
            self.gnn_encoder = SimplifiedGNN(config, edge_index)  # Using Simplified GNN
        else:
            self.gnn_encoder = LGConv(normalize=True)  # Default to LGConv
    ...
    def compute(self, x=None, edge_index=None, perturbed=False, mess_drop=False, layer_num=None):
        ...
        for layer in range(layers):
            if edge_index == None:
                emb = self.gnn_encoder(emb, self.edge_index)  # Use the GNN Encoder here
            else:
                emb = self.gnn_encoder(emb, edge_index)
            if perturbed:
